...
public class MobileModuleServiceImpl extends ServiceImpl<MobileModuleMapper, MobileModule> implements MobileModuleService {
    ...
    public Page<MobileModule> page(MobileModulePageParam mobileModulePageParam) {
        ...
        if (ObjectUtil.isNotEmpty(mobileModulePageParam.getSearchKey())) {
            queryWrapper.like(MobileModule::getTitle, mobileModulePageParam.getSearchKey());
        }
        if (ObjectUtil.isAllNotEmpty(mobileModulePageParam.getSortField(), mobileModulePageParam.getSortOrder())) {
            ...
            queryWrapper.orderBy(StrUtil.toUnderlineCase(mobileModulePageParam.getSortField()), mobileModulePageParam.getSortOrder().equals(CommonSortOrderEnum.ASC.getValue()));
        } else {
            queryWrapper.orderBy(MobileModule::getSortCode);
        }
    ...
    public void add(MobileModuleAddParam mobileModuleAddParam) {
        ...
        boolean repeatTitle = this.count(new QueryWrapper().eq(MobileModule::getCategory,
    ...
    public void delete(List<MobileModuleIdParam> mobileModuleIdParamList) {
        ...
        if (ObjectUtil.isNotEmpty(mobileModuleIdList)) {
            // 获取模块下的菜单
            List<MobileMenu> allMenuList = mobileMenuService.list(new QueryWrapper()
    ...
    public List<JSONObject> mobileModuleSelector() {
        QueryWrapper queryWrapper = new QueryWrapper();
        queryWrapper.select(MobileModule::getId, MobileModule::getTitle);
        queryWrapper.eq(MobileModule::getCategory, MobileResourceCategoryEnum.MODULE.getValue());
        queryWrapper.orderBy(MobileModule::getSortCode);
        return this.list(queryWrapper).stream().map(item -> JSONUtil.createObj().set("id", item.getId()).set("name", item.getTitle()))
