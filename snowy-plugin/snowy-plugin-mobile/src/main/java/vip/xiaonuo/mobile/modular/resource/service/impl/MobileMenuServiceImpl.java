...
public class MobileMenuServiceImpl extends ServiceImpl<MobileMenuMapper, MobileMenu> implements MobileMenuService {
    ...
    public List<Tree<String>> tree(MobileMenuTreeParam mobileMenuTreeParam) {
        ...
        if (ObjectUtil.isNotEmpty(mobileMenuTreeParam.getModule())) {
            queryWrapper.eq(MobileMenu::getModule, mobileMenuTreeParam.getModule());
        }
        if (ObjectUtil.isNotEmpty(mobileMenuTreeParam.getSearchKey())) {
            queryWrapper.like(MobileMenu::getTitle, mobileMenuTreeParam.getSearchKey());
        }
        queryWrapper.eq(MobileMenu::getCategory, MobileResourceCategoryEnum.MENU.getValue());
        queryWrapper.orderBy(MobileMenu::getSortCode);
        List<MobileMenu> mobileMenuList = this.list(queryWrapper);
    ...
    public void add(MobileMenuAddParam mobileMenuAddParam) {
        ...
        boolean repeatTitle = this.count(new QueryWrapper().eq(MobileMenu::getParentId, mobileMenu.getParentId())
        ...
        List<MobileMenu> originDataList = this.list(new QueryWrapper().eq(MobileMenu::getCategory,
    ...
    public void changeModule(MobileMenuChangeModuleParam mobileMenuChangeModuleParam) {
        ...
        List<MobileMenu> mobileMenuList = this.list(new QueryWrapper().eq(MobileMenu::getCategory,
                MobileResourceCategoryEnum.MENU.getValue()));
        ...
        mobileMenuChildList.forEach(mobileMenuTemp -> {
            boolean repeatTitle = this.count(QueryWrapper.create().eq(MobileMenu::getParentId, mobileMenuTemp.getParentId())
        ...
        this.updateBatch(mobileMenuChildList);
    }
    ...
    public void delete(List<MobileMenuIdParam> mobileMenuIdParamList) {
        ...
        if (ObjectUtil.isNotEmpty(mobileMenuIdList)) {
            // 获取菜单下的菜单
            List<MobileMenu> allMenuList = this.list(new QueryWrapper()
    ...
    public List<MobileModule> moduleSelector(MobileMenuSelectorModuleParam mobileMenuSelectorModuleParam) {
        ...
        if (ObjectUtil.isNotEmpty(mobileMenuSelectorModuleParam.getSearchKey())) {
            queryWrapper.like(MobileModule::getTitle, mobileMenuSelectorModuleParam.getSearchKey());
        }
        queryWrapper.eq(MobileModule::getCategory, MobileResourceCategoryEnum.MODULE.getValue());
        return mobileModuleService.list(queryWrapper);
    }
    ...
    public List<Tree<String>> menuTreeSelector(MobileMenuSelectorMenuParam mobileMenuSelectorMenuParam) {
        ...
        if (ObjectUtil.isNotEmpty(mobileMenuSelectorMenuParam.getModule())) {
            queryWrapper.eq(MobileMenu::getModule, mobileMenuSelectorMenuParam.getModule());
        }
        List<MobileMenu> resourceList = this.list(queryWrapper);
    ...
    public List<Tree<String>> loginMobileMenuTree(List<String> menuIdList) {
        ...
        List<MobileMenu> allModuleAndMenuAndSpaList = this.list(new QueryWrapper()
                .in(MobileMenu::getCategory, MobileResourceCategoryEnum.MODULE.getValue(), MobileResourceCategoryEnum.MENU.getValue())
                .orderBy(MobileMenu::getCategory).asc().orderBy(MobileMenu::getSortCode).asc());
        // 全部以菜单承载
