...
public class DevLogUtil {
    ...
    private static final DevLogService devLogService = Solon.context().getBean(DevLogService.class);
    ...
    public static void executeOperationLog(CommonLog commonLog, String userName, Invocation inv, String resultJson) {
        ...
        ThreadUtil.execute(() -> {
            ...
            devLog.setClassName(inv.target().getClass().getName());
            devLog.setMethodName(inv.method().getMethod().getName());
            devLog.setReqUrl(request.url());
            devLog.setParamJson(CommonJoinPointUtil.getArgsJsonString(inv.args()));
     ...
     * @date 2022/9/2 15:31
     */
    public static void executeExceptionLog(CommonLog commonLog, String userName, Invocation inv, Exception exception) {
        Context request = CommonServletUtil.getRequest();
        String loginId = StpUtil.getLoginIdAsString();
        if (ObjectUtil.isEmpty(loginId)) {
            loginId = "-1";
        ...
        ThreadUtil.execute(() -> {
            ...
            devLog.setClassName(inv.target().getClass().getName());
            devLog.setMethodName(inv.method().getMethod().getName());
            devLog.setReqMethod(request.method());
            devLog.setReqUrl(request.url());
            devLog.setParamJson(CommonJoinPointUtil.getArgsJsonString(inv.args()));
            devLog.setOpTime(DateTime.now());
            devLog.setOpUser(userName);
