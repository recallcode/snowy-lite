...
public class BizOrgServiceImpl extends ServiceImpl<BizOrgMapper, BizOrg> implements BizOrgService {
    ...
    public Page<BizOrg> page(BizOrgPageParam bizOrgPageParam) {
        ...
        if(ObjectUtil.isNotEmpty(bizOrgPageParam.getParentId())) {
            queryWrapper.eq(BizOrg::getParentId, bizOrgPageParam.getParentId());
        }
        if(ObjectUtil.isNotEmpty(bizOrgPageParam.getSearchKey())) {
            queryWrapper.like(BizOrg::getName, bizOrgPageParam.getSearchKey());
        }
        if(ObjectUtil.isAllNotEmpty(bizOrgPageParam.getSortField(), bizOrgPageParam.getSortOrder())) {
            ...
            queryWrapper.orderBy(StrUtil.toUnderlineCase(bizOrgPageParam.getSortField()),bizOrgPageParam.getSortOrder().equals(CommonSortOrderEnum.ASC.getValue()));
        } else {
    ...
    public void add(BizOrgAddParam bizOrgAddParam) {
        ...
        // 重复名称
        boolean repeatName = this.count(new QueryWrapper().eq(BizOrg::getParentId, bizOrg.getParentId())
    ...
    public void delete(List<BizOrgIdParam> bizOrgIdParamList) {
        ...
        if(ObjectUtil.isNotEmpty(orgIdList)) {
            ...
            // 机构下有人不能删除（直属机构）
            boolean hasOrgUser = bizUserService.count(new QueryWrapper().in(BizUser::getOrgId, toDeleteOrgIdList)) > 0;
            if(hasOrgUser) {
                throw new CommonException("请先删除机构下的人员");
            }
            // 机构下有人不能删除（兼任机构）
            List<String> positionJsonList = bizUserService.list(new QueryWrapper()
            ...
            // 机构下有岗位不能删除
            boolean hasPosition = bizPositionService.count(new QueryWrapper().in(BizPosition::getOrgId, toDeleteOrgIdList)) > 0;
            if(hasPosition) {
    ...
    public List<Tree<String>> orgTreeSelector() {
        ...
        queryWrapper.orderBy(BizOrg::getSortCode);
        List<BizOrg> bizOrgList = this.list(queryWrapper);
    ...
    public List<BizOrg> orgListSelector(BizOrgSelectorOrgListParam bizOrgSelectorOrgListParam) {
        ...
        if(ObjectUtil.isNotEmpty(bizOrgSelectorOrgListParam.getParentId())) {
            queryWrapper.eq(BizOrg::getParentId, bizOrgSelectorOrgListParam.getParentId());
        }
        if(ObjectUtil.isNotEmpty(bizOrgSelectorOrgListParam.getSearchKey())) {
            queryWrapper.like(BizOrg::getName, bizOrgSelectorOrgListParam.getSearchKey());
        }
        queryWrapper.orderBy(BizOrg::getSortCode);
        return this.list(queryWrapper);
    }
    ...
    public Page<BizUser> userSelector(BizOrgSelectorUserParam bizOrgSelectorUserParam) {
        ...
        if(ObjectUtil.isNotEmpty(bizOrgSelectorUserParam.getSearchKey())) {
            queryWrapper.like(BizUser::getName, bizOrgSelectorUserParam.getSearchKey());
        }
