...
public class SysOrgServiceImpl extends ServiceImpl<SysOrgMapper, SysOrg> implements SysOrgService {
    ...
    public Page<SysOrg> page(SysOrgPageParam sysOrgPageParam) {
        ...
        if (ObjectUtil.isNotEmpty(sysOrgPageParam.getParentId())) {
            queryWrapper.eq(SysOrg::getParentId, sysOrgPageParam.getParentId());
        }
        if (ObjectUtil.isNotEmpty(sysOrgPageParam.getSearchKey())) {
            queryWrapper.like(SysOrg::getName, sysOrgPageParam.getSearchKey());
        }
        if (ObjectUtil.isAllNotEmpty(sysOrgPageParam.getSortField(), sysOrgPageParam.getSortOrder())) {
            ...
            queryWrapper.orderBy(StrUtil.toUnderlineCase(sysOrgPageParam.getSortField()), sysOrgPageParam.getSortOrder().equals(CommonSortOrderEnum.ASC.getValue()));
        } else {
    ...
    public void add(SysOrgAddParam sysOrgAddParam) {
        ...
        // 重复名称
        boolean repeatName = this.count(new QueryWrapper().eq(SysOrg::getParentId, sysOrg.getParentId())
    ...
    public void delete(List<SysOrgIdParam> sysOrgIdParamList) {
        ...
        if (ObjectUtil.isNotEmpty(orgIdList)) {
            ...
            // 组织下有人不能删除（直属组织）
            boolean hasOrgUser = sysUserService.count(new QueryWrapper().in(SysUser::getOrgId, toDeleteOrgIdList)) > 0;
            if (hasOrgUser) {
            ...
            // 组织下有角色不能删除
            boolean hasRole = sysRoleService.count(new QueryWrapper().in(SysRole::getOrgId, toDeleteOrgIdList)) > 0;
            if (hasRole) {
                throw new CommonException("请先删除组织下的角色");
            }
            // 组织下有职位不能删除
            boolean hasPosition = sysPositionService.count(new QueryWrapper().in(SysPosition::getOrgId, toDeleteOrgIdList)) > 0;
            if (hasPosition) {
    ...
    public Page<SysOrg> orgListSelector(SysOrgSelectorOrgListParam sysOrgSelectorOrgListParam) {
        ...
        if (ObjectUtil.isNotEmpty(sysOrgSelectorOrgListParam.getParentId())) {
            queryWrapper.eq(SysOrg::getParentId, sysOrgSelectorOrgListParam.getParentId());
        }
        if (ObjectUtil.isNotEmpty(sysOrgSelectorOrgListParam.getSearchKey())) {
            queryWrapper.like(SysOrg::getName, sysOrgSelectorOrgListParam.getSearchKey());
        }
        queryWrapper.orderBy(SysOrg::getSortCode);
        return this.page(CommonPageRequest.defaultPage(), queryWrapper);
    }
    ...
    public Page<SysUser> userSelector(SysOrgSelectorUserParam sysOrgSelectorUserParam) {
        ...
        if (ObjectUtil.isAllEmpty(sysOrgSelectorUserParam.getOrgId(), sysOrgSelectorUserParam.getSearchKey())) {
            ...
            if (ObjectUtil.isNotEmpty(sysOrgSelectorUserParam.getSearchKey())) {
                queryWrapper.like(SysUser::getName, sysOrgSelectorUserParam.getSearchKey());
            }
