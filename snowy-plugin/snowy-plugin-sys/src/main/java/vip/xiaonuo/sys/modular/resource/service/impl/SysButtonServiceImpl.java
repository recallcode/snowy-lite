...
public class SysButtonServiceImpl extends ServiceImpl<SysButtonMapper, SysButton> implements SysButtonService {
    ...
    public Page<SysButton> page(SysButtonPageParam sysButtonPageParam) {
        ...
        queryWrapper.eq(SysButton::getCategory, SysResourceCategoryEnum.BUTTON.getValue());
        if (ObjectUtil.isNotEmpty(sysButtonPageParam.getParentId())) {
            queryWrapper.eq(SysButton::getParentId, sysButtonPageParam.getParentId());
        }
        if (ObjectUtil.isNotEmpty(sysButtonPageParam.getSearchKey())) {
            queryWrapper.like(SysButton::getTitle, sysButtonPageParam.getSearchKey());
        }
        if (ObjectUtil.isAllNotEmpty(sysButtonPageParam.getSortField(), sysButtonPageParam.getSortOrder())) {
            ...
            queryWrapper.orderBy(StrUtil.toUnderlineCase(sysButtonPageParam.getSortField()), sysButtonPageParam.getSortOrder().equals(CommonSortOrderEnum.ASC.getValue()));
        } else {
            queryWrapper.orderBy(SysButton::getSortCode);
        }
    ...
    public void delete(List<SysButtonIdParam> sysButtonIdParamList) {
        ...
        if (ObjectUtil.isNotEmpty(buttonIdList)) {
            // 获取按钮的父菜单id集合
            List<String> parentMenuIdList = sysMenuService.list(new QueryWrapper().in(SysMenu::getId, buttonIdList)
            ...
            if (ObjectUtil.isNotEmpty(parentMenuIdList)) {
                sysRelationService.list(new QueryWrapper().in(SysRelation::getTargetId, parentMenuIdList)
                    ...
                    sysRelationService.updateChain().eq(SysRelation::getId, sysRelation.getId())
                            .set(SysRelation::getExtJson, JSONUtil.toJsonStr(extJsonObject)).update();
                });
                // 执行删除
