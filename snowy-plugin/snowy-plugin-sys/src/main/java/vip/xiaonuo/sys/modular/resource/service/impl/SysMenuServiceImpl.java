...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuMapper, SysMenu> implements SysMenuService {
    ...
    public Page<SysMenu> page(SysMenuPageParam sysMenuPageParam) {
        ...
        queryWrapper.eq(SysMenu::getCategory, SysResourceCategoryEnum.MENU.getValue());
        if (ObjectUtil.isNotEmpty(sysMenuPageParam.getSearchKey())) {
            queryWrapper.like(SysMenu::getTitle, sysMenuPageParam.getSearchKey());
        }
        if (ObjectUtil.isNotEmpty(sysMenuPageParam.getModule())) {
            queryWrapper.like(SysMenu::getModule, sysMenuPageParam.getModule());
        }
        if (ObjectUtil.isAllNotEmpty(sysMenuPageParam.getSortField(), sysMenuPageParam.getSortOrder())) {
            ...
            queryWrapper.orderBy(StrUtil.toUnderlineCase(sysMenuPageParam.getSortField()), sysMenuPageParam.getSortOrder().equals(CommonSortOrderEnum.ASC.getValue()));
        } else {
            queryWrapper.orderBy(SysMenu::getSortCode);
        }
    ...
    public List<Tree<String>> tree(SysMenuTreeParam sysMenuTreeParam) {
        QueryWrapper queryWrapper = new QueryWrapper();
        queryWrapper.eq(SysMenu::getCategory, SysResourceCategoryEnum.MENU.getValue())
                .orderBy(SysMenu::getSortCode);
        if (ObjectUtil.isNotEmpty(sysMenuTreeParam.getModule())) {
            queryWrapper.eq(SysMenu::getModule, sysMenuTreeParam.getModule());
        }
        if (ObjectUtil.isNotEmpty(sysMenuTreeParam.getSearchKey())) {
            queryWrapper.like(SysMenu::getTitle, sysMenuTreeParam.getSearchKey());
        }
    ...
    private void fillParentSysMenuInfo(List<SysMenu> resourceList) {
        if (CollUtil.isNotEmpty(resourceList)) {
            ...
            if (CollUtil.isNotEmpty(parentIds)) {
                QueryWrapper queryWrapper = new QueryWrapper();
                queryWrapper.in(SysMenu::getId, parentIds);
                List<SysMenu> parentSysMenus = this.list(queryWrapper);
    ...
    public void add(SysMenuAddParam sysMenuAddParam) {
        ...
        boolean repeatTitle = this.count(new QueryWrapper().eq(SysMenu::getParentId, sysMenu.getParentId())
        ...
        List<SysMenu> originDataList = this.list(new QueryWrapper().eq(SysMenu::getCategory,
    ...
    public String addForGenMenu(String parentId, String busName, String title, String module, String path) {
        ...
        SysMenu sysOldMenu = this.getOne(new QueryWrapper().eq(SysMenu::getTitle, title)
    ...
    public void changeModule(SysMenuChangeModuleParam sysMenuChangeModuleParam) {
        ...
        List<SysMenu> sysMenuList = this.list(new QueryWrapper().eq(SysMenu::getCategory,
                SysResourceCategoryEnum.MENU.getValue()));
        ...
        sysMenuChildList.forEach(sysMenuTemp -> {
            boolean repeatTitle = this.count(QueryWrapper.create().eq(SysMenu::getParentId, sysMenuTemp.getParentId())
    ...
    public void delete(List<SysMenuIdParam> sysMenuIdParamList) {
        ...
        if (ObjectUtil.isNotEmpty(sysMenuIdList)) {
            // 获取菜单下的菜单、按钮
            List<SysMenu> allMenuList = this.list(new QueryWrapper()
    ...
    public List<SysModule> moduleSelector(SysMenuSelectorModuleParam sysMenuSelectorModuleParam) {
        ...
        if (ObjectUtil.isNotEmpty(sysMenuSelectorModuleParam.getSearchKey())) {
            queryWrapper.like(SysModule::getTitle, sysMenuSelectorModuleParam.getSearchKey());
        }
        queryWrapper.eq(SysModule::getCategory, SysResourceCategoryEnum.MODULE.getValue());
        return sysModuleService.list(queryWrapper);
    }
    ...
    public List<Tree<String>> menuTreeSelector(SysMenuSelectorMenuParam sysMenuSelectorMenuParam) {
        ...
        if (ObjectUtil.isNotEmpty(sysMenuSelectorMenuParam.getModule())) {
            queryWrapper.eq(SysMenu::getModule, sysMenuSelectorMenuParam.getModule());
        }
        List<SysMenu> resourceList = this.list(queryWrapper);
        List<TreeNode<String>> treeNodeList = resourceList.stream().map(sysMenu ->
                        new TreeNode<>(sysMenu.getId(), sysMenu.getParentId(),
