...
public class SysRoleServiceImpl extends ServiceImpl<SysRoleMapper, SysRole> implements SysRoleService {
    ...
    public Page<SysRole> page(SysRolePageParam sysRolePageParam) {
        ...
        if (ObjectUtil.isNotEmpty(sysRolePageParam.getOrgId())) {
            queryWrapper.eq(SysRole::getOrgId, sysRolePageParam.getOrgId());
        }
        if (ObjectUtil.isNotEmpty(sysRolePageParam.getCategory())) {
            queryWrapper.eq(SysRole::getCategory, sysRolePageParam.getCategory());
        }
        if (ObjectUtil.isNotEmpty(sysRolePageParam.getSearchKey())) {
            queryWrapper.like(SysRole::getName, sysRolePageParam.getSearchKey());
        }
        if (ObjectUtil.isAllNotEmpty(sysRolePageParam.getSortField(), sysRolePageParam.getSortOrder())) {
            ...
            queryWrapper.orderBy(StrUtil.toUnderlineCase(sysRolePageParam.getSortField()), sysRolePageParam.getSortOrder().equals(CommonSortOrderEnum.ASC.getValue()));
        } else {
    ...
    public void add(SysRoleAddParam sysRoleAddParam) {
        ...
        boolean repeatName = this.count(new QueryWrapper().eq(SysRole::getOrgId, sysRole.getOrgId())
    ...
    public void delete(List<SysRoleIdParam> sysRoleIdParamList) {
        ...
        if (ObjectUtil.isNotEmpty(sysRoleIdList)) {
            ...
            // 级联删除角色与用户关系
            sysRelationService.remove(new QueryWrapper().in(SysRelation::getTargetId, sysRoleIdList)
                    .eq(SysRelation::getCategory, SysRelationCategoryEnum.SYS_USER_HAS_ROLE.getValue()));
            // 级联删除角色与资源关系
            sysRelationService.remove(new QueryWrapper().in(SysRelation::getObjectId, sysRoleIdList)
    ...
    public List<SysRoleGrantResourceTreeResult> resourceTreeSelector() {
        QueryWrapper queryWrapper = new QueryWrapper();
        queryWrapper.in(SysMenu::getCategory, SysResourceCategoryEnum.MODULE.getValue(), SysResourceCategoryEnum.MENU.getValue(),
                SysResourceCategoryEnum.BUTTON.getValue());
        List<SysMenu> allMenuAndButtonAndFieldList = sysMenuService.list(queryWrapper);
            ...
            if (routing.target() instanceof Action) {
                ...
                if (ObjectUtil.isNotEmpty(saCheckPermission)) {
                    ...
                    if (pathExpr != null) {
                        ...
                        ApiOperation apiOperation = action.method().getAnnotation(ApiOperation.class);
                        ...
                        permissionResult.add(pathExpr + StrUtil.BRACKET_START + apiName + StrUtil.BRACKET_END);
                    }
                }
            }
        }
    ...
    public Page<SysRole> roleSelector(SysRoleSelectorRoleParam sysRoleSelectorRoleParam) {
        ...
        if (ObjectUtil.isNotEmpty(sysRoleSelectorRoleParam.getOrgId())) {
            queryWrapper.eq(SysRole::getOrgId, sysRoleSelectorRoleParam.getOrgId());
        }
        if (ObjectUtil.isNotEmpty(sysRoleSelectorRoleParam.getCategory())) {
            queryWrapper.eq(SysRole::getCategory, sysRoleSelectorRoleParam.getCategory());
        }
        if (ObjectUtil.isNotEmpty(sysRoleSelectorRoleParam.getSearchKey())) {
            queryWrapper.like(SysRole::getName, sysRoleSelectorRoleParam.getSearchKey());
        }
        if (ObjectUtil.isNotEmpty(sysRoleSelectorRoleParam.getDataScopeList())) {
            queryWrapper.in(SysRole::getOrgId, sysRoleSelectorRoleParam.getDataScopeList());
        }
        // 排除超管角色
        if (sysRoleSelectorRoleParam.isExcludeSuperAdmin()) {
            queryWrapper.ne(SysRole::getCode, SysBuildInEnum.BUILD_IN_ROLE_CODE.getValue());
        }
        queryWrapper.orderBy(SysRole::getSortCode);
        return this.page(CommonPageRequest.defaultPage(), queryWrapper);
    }
    ...
    public Page<SysUser> userSelector(SysRoleSelectorUserParam sysRoleSelectorUserParam) {
        ...
        if (ObjectUtil.isAllEmpty(sysRoleSelectorUserParam.getOrgId(), sysRoleSelectorUserParam.getSearchKey())) {
            ...
            if (ObjectUtil.isNotEmpty(sysRoleSelectorUserParam.getSearchKey())) {
                queryWrapper.like(SysUser::getName, sysRoleSelectorUserParam.getSearchKey());
            }
            queryWrapper.orderBy(SysUser::getSortCode);
            return sysUserService.page(CommonPageRequest.defaultPage(), queryWrapper);
        }
    }
