...
import vip.xiaonuo.common.pojo.CommonPageOrderItem;
...
public class CommonPageRequest {
    ...
    public static <T> Page<T> defaultPage() {
        return defaultPage(null, null);
    }

    public static <T> Page<T> defaultPage(List<CommonPageOrderItem> orderItemList, QueryWrapper query) {
        ...
        if (ObjectUtil.isNotEmpty(orderItemList)) {
            for (CommonPageOrderItem pageOrderItem : orderItemList) {
                query.orderBy(new QueryOrderBy(new QueryColumn(CharSequenceUtil.toUnderlineCase(pageOrderItem.getColumn())),
                        Direction.ASC.name()));
            }
        }
        return objectPage;
